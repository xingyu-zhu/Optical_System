function [E_out, Debug_Data] = OC_VOA_Module(E_in, Params)
...
%   Debug_Data : Structure with power stats
    ...
    if isfield(VOA_Config, 'OutputPower') && ~isempty(VOA_Config.OutputPower)
        ...
        if P_in_W > 0 && target_W > 0
            Att_dB = max(0, 10*log10(P_in_W / target_W));
        end
    end
...
function power_W = opticalPower(E)
    if size(E, 2) == 2
        power_W = mean(sum(abs(E).^2, 2));
    else
        power_W = mean(abs(E).^2);
    end
end
