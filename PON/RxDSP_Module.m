...
%   ResultData : Structure with intermediate signals
    ...
    % The LO is now physically aligned with the signal laser (base frequency).
    % This means the Pilot is near 0 Hz, and the Data is near cf(ch).
    Target_Channel_Freq = Params.cf(ch);
    
    % 1: t_vec 
    t_vec = (0:length(RX)-1);
    
    % The Pilot is already near baseband, so no need for massive shifts to extract it
    RX_shifted_for_pilot = RX.';
    ...
    % The Data is at Target_Channel_Freq + FreOffset0.
    % We shift the raw RX signal by -(Target_Channel_Freq + FreOffset0) to perfectly center the Data at 0 Hz.
    Rsig_X = RX.' .* exp(-1i * 2 * pi * (Target_Channel_Freq + FreOffset0) / Fs_Rx * t_vec);
    Rsig_Y = RY.' .* exp(-1i * 2 * pi * (Target_Channel_Freq + FreOffset0) / Fs_Rx * t_vec);
