...
Params.Opt.Obj.LO.Power      = 20e-3;  % 20 mW (+13 dBm) - Typically higher than signal
Params.Opt.Obj.LO.Linewidth  = 5e6;  % 100 kHz
...
fprintf('  > LO Tuned To (Base + FO): %.2f GHz\n', Params.Opt.Obj.LO.FreqOffset/1e9);
...
% Verify the ROP 
[ROP_dBm, ~] = PowerMeter_Module(E_Rx);
...
if ~ismember(port_n, Plotted_Ports_ADC)
    ...
    fig_rx_adc = figure('Name', sprintf('ONU Rx ADC Spectrum'), 'Color', 'w');
    nfft_rx = 2^15;
    ...
    title(sprintf('ONU接收信号电谱'));
    xlabel('Frequency (GHz)');
    ylabel('Power Spectral Density (dB/Hz)');
    grid on;
    xlim([-40 40]); 

    saveas(fig_rx_adc, fullfile('img', 'down', sprintf('ONU接收信号电谱.png')));
