function [E_LO, Debug_Data] = LO_Optical_Module_v3(TimeVector, Params)
...
%   Debug_Data : Structure containing Phase Noise and Frequency Shift info
    ...
    FreqOffset = LO_Config.FreqOffset;  % Hz%%%
    ...
    % LO typically requires low/no RIN, turn it off by default unless specified
    ...
    Laser_In_Params.EmissionFrequency = Params.LO.EmissionFrequency; % Base frequency (Reference)
    ...
    [E_Base, Laser_Debug] = LaserCW_Module(TimeVector, Laser_In_Params);
    PhaseNoise_LO = Laser_Debug.PhaseNoise1;
%     save('PN.mat', 'PhaseNoise2', '-append') 
    ...
    Freq_Shift_Term = exp(1j * 2 * pi * (-FreqOffset) * TimeVector); %%%LO base
    ...
    %% 5. Pack Debug Data
    Debug_Data.PhaseNoise   = PhaseNoise_LO; % Inherit phase noise from LaserCW
