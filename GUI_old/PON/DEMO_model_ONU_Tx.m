function [Params, E_Total, SigX_Full, SigY_Full] = DEMO_model_ONU_Tx(num_ONU, Params)
addpath('./component'); 
...
for k = 1:Params.num_ONUs
    ...
    rf_in_y = complex(Vpi.*asin(real(rfall_y))./pi, Vpi.*asin(imag(rfall_y))./pi);
        ...
        % --- 添加 20.48ns 的 Burst Overhead ---
    T_oh = 20.48e-9;
    N_oh_sym = round(T_oh * Params.BaudRate);
    oh_sym_x = qammod(randi([0 3], N_oh_sym, 1), 4);
    oh_sym_y = qammod(randi([0 3], N_oh_sym, 1), 4);
    
    % 重采样并进行频偏上变频，使其与 Payload 格式一致
    oh_sig_x = resample(oh_sym_x, Params.Fs_Tx, Params.BaudRate);
    oh_sig_y = resample(oh_sym_y, Params.Fs_Tx, Params.BaudRate);
    
    t_oh = (0:length(oh_sig_x)-1).' / Params.Fs_Tx;
    oh_sig_x = oh_sig_x .* exp(1j * 2 * pi * Params.cf(1) * t_oh);
    oh_sig_y = oh_sig_y .* exp(1j * 2 * pi * Params.cf(1) * t_oh);
    
    % 功率对齐并拼接到有效数据前
    oh_sig_x = oh_sig_x * sqrt(mean(abs(rf_in_x).^2) / mean(abs(oh_sig_x).^2));
    oh_sig_y = oh_sig_y * sqrt(mean(abs(rf_in_y).^2) / mean(abs(oh_sig_y).^2));
    
    rf_in_x = [oh_sig_x; rf_in_x];
    rf_in_y = [oh_sig_y; rf_in_y];
    % -----------------------------------------------------------
    
    Params_ONU.Driver.Bandwidth = Params_ONU.Driver.Bandwidth;
    Params_ONU.Driver.Gain_dB = Params_ONU.Driver.Gain_dB;
    [rf_out_x, rf_out_y] = Driver_Module(rf_in_x, rf_in_y, Params_ONU);
    
    % Laser (Independent Phase Noise)
    LaserParam_ONU = struct();
    LaserParam_ONU.EmissionFrequency = Params.LaserParam.EmissionFrequency; 
    LaserParam_ONU.AveragePower      = Params.LaserParam.AveragePower;    
    LaserParam_ONU.Linewidth         = Params.LaserParam.Linewidth;    
    LaserParam_ONU.Azimuth = 45; LaserParam_ONU.Ellipticity = 0;
    LaserParam_ONU.IncludeRIN = 'ON'; LaserParam_ONU.RIN = Params.LaserParam.RIN;
    ...
    if k == 1
        Params.Overhead_Samples_Tx = length(oh_sig_x);
...
saveas(fig_tx_spec, fullfile('img/up', 'Tx_Optical_Spectrum.png'));
% ===============================================================
end