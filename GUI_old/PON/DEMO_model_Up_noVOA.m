function [SNR, BER] = DEMO_model_Up(num_ONU, Params)
...
for pt_idx = 1:length(PTx_dBm_List)
    ...
    % 2. 扫描 ROP 
    [BER_Array, ~,BER, SNR, Constellation_Final] = Evaluate_BER_vs_ROP_Uplink(E_Total_Scaled, SigX_Full, SigY_Full, Params, ROP_dBm_List, Actual_PTx_dBm);
    % 画星座图
...
%% ==================  ROP 扫描与 BER 获取 ==================
function [BER_Array, SNR_Array, BER_est, SNR_est, ResData_All] = Evaluate_BER_vs_ROP_Uplink(E_Total_Scaled, SigX_Full, SigY_Full, Params, ROP_dBm_List, Current_PTx_dBm)
