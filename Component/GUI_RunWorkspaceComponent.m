...
function ws = runTxDSP(ws, fn, Params)
    ...
    [rf_i, rf_q, ms] = TxImbalance_Module(x_t, y_t, Params);
    if strcmp(getContext(ws.Context, 'component_type', ''), 'onutxdsp')
...
function ws = runModulator(ws, fn, Params)
    ...
    if ~isempty(fn) && exist('EDFA_Module', 'file') == 2
        [ws.E_out, ws.AmpDebug] = EDFA_Module(ws.E_Modulator_Out, Params);
    else
...
function ws = runFiber(ws, fn, Params)
    e = firstOpticalField(ws);
    if isempty(e), ws = waitFor(ws, 'optical field'); return; end
    if strcmp(char(fn), 'Fiber_Channel')
...
function ws = runICR(ws, fn, Params)
    ...
    if ~isempty(fn) && exist('PolarizationEmulation_Module', 'file') == 2 && ~isstruct(lo)
        [lo, ws.SOP_Data] = PolarizationEmulation_Module(lo, sig, Params);
        ws.E_LO_Rot = lo;
    end
...
function Params = buildDefaultParams(guiParams, context)
    ...
    if exist('DefineEle_platform', 'file') == 2
        Params.Ele.Obj = DefineEle_platform( ...
    ...
    Params.EnableMatlabPlots = boolParam(guiParams, 'EnableMatlabPlots', false);
...
function Params = applyUplinkFrequencyPlan(Params, guiParams, context)
    ...
    % Match the legacy DEMO_model_ONU_Tx.m uplink SC plan:
...
function cf = buildUplinkCarrierOffset(guiParams)
    % Match the uplink frequency plan in legacy DEMO_model_ONU_Tx.m:
