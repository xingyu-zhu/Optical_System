...
%         title(sprintf('恢复的星座图 (PTx = %.2f dBm)', Actual_PTx_dBm));
        title(sprintf('恢复的星座图'));
        xlabel('In-Phase'); ylabel('Quadrature');
        grid on; axis square;
        if ~exist(fullfile('img', 'up'), 'dir'), mkdir(fullfile('img', 'up')); end
        saveas(fig_const, fullfile('img', 'up', '恢复的星座图.png'));
    end
    ...
    fprintf('\n => Link Budget (dB) @ Actual PTx=%.2f dBm: ONU = %.2f dB\n', Actual_PTx_dBm, LB_Array(pt_idx));
end
...
function Plot_Power_Budget_Uplink(Actual_PTx_dBm_List, LB_Array, Target_ONU)
    ...
    title('ONU发射光功率与链路预算曲线', 'FontSize', 14);
