function [SNR, BER, ResData] = RxDSP_Module_up(Burst_Rx_X, Burst_Rx_Y, SigX_Full, SigY_Full, Params)
    ...
    if isempty(Plotted_Sync)
        fig_sync = figure('Name', 'Frame Synchronization Metric', 'Color', 'w', 'Position', [200, 200, 700, 450]);
...
%     loop_delay_symbols = 6000; 
    ...
    fig_mse = figure('Name', 'MSE Convergence', 'Color', 'w', 'Position', [150, 150, 600, 400]);
...
%     text(loop_delay_symbols/2, max(Smoothed_MSE)*0.95, 'Loop\nDelay', 'HorizontalAlignment', 'center', 'FontSize', 11);
    ...
    if isempty(Plotted_Const_MIMO)
        fig_const = figure('Name', 'Equalized Constellation', 'Color', 'w', 'Position', [200, 250, 450, 450]);
